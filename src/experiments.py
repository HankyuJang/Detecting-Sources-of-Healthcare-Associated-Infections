...
$ python experiments.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 
"""
...
def prepare_result_dataframes():
    ...
    return df_GT, df_BR, df_G1, df_ISCK, df_GR

def concat_result_dataframes():
    index_list = ["GT", "Random", "P1-LazyGreedy", "P2-LazyISCK", "P3-LazyGreedyRatio"]
    df_result = pd.concat([df_GT, df_BR, df_G1, df_ISCK, df_GR])
...
def save_result_dataframes(name, k_total):
    # Save datasets
    df_GT.to_csv("../tables/exp/{}/k{}/GT.csv".format(name, k_total), index=False)
    df_BR.to_csv("../tables/exp/{}/k{}/BR.csv".format(name, k_total), index=False)
    df_G1.to_csv("../tables/exp/{}/k{}/G1.csv".format(name, k_total), index=False)
    df_ISCK.to_csv("../tables/exp/{}/k{}/ISCK.csv".format(name, k_total), index=False)
    df_GR.to_csv("../tables/exp/{}/k{}/GR.csv".format(name, k_total), index=False)

    df_result.to_csv("../tables/exp/{}/k{}/result_concat.csv".format(name, k_total), index=True)
    df_ground_truth_observations.to_csv("../tables/exp/{}/k{}/GT_observations.csv".format(name, k_total), index=False)
...
def print_result_dataframes():
    ...
    print("\nBaseline. Random")
    print(df_BR.round(2))
    print("\nProblem1: Greedy w/ cardinality constraint")
    print(df_G1.round(2))
    print("\nProblem2: ISCK")
...
if __name__ == "__main__":
    ...
    GT_loss_1, GT_loss_total, \
        ...
        compute_GT_loss_per_timestep(simul, seeds_array, obs_state, list_of_sets_of_P, list_of_sets_of_N, n_t_for_eval)
    ...
    # NOTE: Need to modify Degree baseline
    ...
    ISCK_seeds_array, ISCK_n_S, ISCK_n_S_correct, ISCK_loss_1, ISCK_loss_total, \
        ...
        ISCK_TP, ISCK_TN, ISCK_FP, ISCK_FN, ISCK_F1, ISCK_MCC = \
            run_ISCK_report_loss_per_timestep(simul, list_of_people_idx_arrays, number_of_seeds_over_time, \
                seeds_array, obs_state, list_of_sets_of_P, list_of_sets_of_N, n_t_for_eval, \
                array_of_knapsack_constraints_on_f, flag_lazy=True, flag_knapsack_in_pi=True, n_ISCK_iter=1)
    stop = timeit.default_timer()
