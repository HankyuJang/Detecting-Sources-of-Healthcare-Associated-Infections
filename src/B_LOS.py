...
Description: This script runs LOS baseline on the time expanded graphs

Usage

- 4 graphs for UIHC sampled

$ python B_LOS.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -sampled True -dose_response exponential -seeds_per_t 1
$ python B_LOS.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -sampled True -dose_response exponential -seeds_per_t 3
$ python B_LOS.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -sampled True -dose_response linear -seeds_per_t 1
$ python B_LOS.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -sampled True -dose_response linear -seeds_per_t 3

- 2 graphs for UIHC whole

$ python B_LOS.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -dose_response exponential -seeds_per_t 1
$ python B_LOS.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -dose_response exponential -seeds_per_t 3

- 2 graphs for G_Carilion

$ python B_LOS.py -name G_Carilion -dose_response exponential -seeds_per_t 1
$ python B_LOS.py -name G_Carilion -dose_response exponential -seeds_per_t 3
"""
...
if __name__ == "__main__":
    ...
    # Step2: For each candidate seeds, count the number of terminal nodes that are LOS.
    LOS_dict = dict()
    for v in candidate_seed_list:
        LOS_dict[v] = 1

    for node, time in candidate_seed_list:
        # print(v)
        v = (node, time)
        t_temp = time
        while True:
            ...
            candidate_seed_at_t_temp = (node, t_temp)
            # print(candidate_seed_at_t_temp)
            if G.out_degree(candidate_seed_at_t_temp) >= 2:
                LOS_dict[v] += 1
            else:
                break
    ...
    unique_values = np.unique(np.array(list(LOS_dict.values())))
    unique_values_sorted = np.sort(unique_values)[::-1] # sort in decreasing order

    # Step3: Sort candidate seeds in terms of the number of LOS nodes. Shuffle within those with the same number of seeds
    total_seed_list = [] # This list of seeds are randomly sorted within the seeds that have same LOS.
    for value in unique_values_sorted:
        seed_list_w_same_value = [v for v in LOS_dict if LOS_dict[v] == value]
        random.shuffle(seed_list_w_same_value)
        total_seed_list.extend(seed_list_w_same_value)
    end = timer()
    B_LOS_time_elapsed = get_elapsed_time(start, end)

    print([(v, LOS_dict[v]) for v in total_seed_list])
    ...
    for idx_seed_set_size, seed_set_size in enumerate(seed_set_size_constraint_list):
        ...
        B_LOS_seed_list = total_seed_list[:seed_set_size]

        B_LOS_seeds_array = np.zeros((seeds_array.shape)).astype(bool)

        for seed_name, time in B_LOS_seed_list:
            seed_idx = node_name_to_idx_mapping[seed_name]
            B_LOS_seeds_array[time, seed_idx] = True

        _, B_LOS_n_S, B_LOS_n_S_correct, B_LOS_loss_1, B_LOS_loss_total, \
            B_LOS_list_of_P_hit, B_LOS_list_of_N_hit, \
            B_LOS_TP, B_LOS_TN, B_LOS_FP, B_LOS_FN, B_LOS_F1, B_LOS_MCC = \
                evaluate_solution_seeds(simul, list_of_people_idx_arrays, seeds_array, B_LOS_seeds_array, obs_state, list_of_sets_of_P, list_of_sets_of_N, args.n_t_for_eval)

        L_B_LOS_S_detected.append(str(list(B_LOS_seeds_array.nonzero()[1])))
        L_B_LOS_S_timesteps.append(str(list(B_LOS_seeds_array.nonzero()[0])))

        L_B_LOS_n_S.append(B_LOS_n_S)
        L_B_LOS_n_S_correct.append(B_LOS_n_S_correct)
        L_B_LOS_TP.append(B_LOS_TP)
        L_B_LOS_TN.append(B_LOS_TN)
        L_B_LOS_FP.append(B_LOS_FP)
        L_B_LOS_FN.append(B_LOS_FN)
        L_B_LOS_F1.append(B_LOS_F1)
        L_B_LOS_MCC.append(B_LOS_MCC)
        L_B_LOS_time_elapsed.append(B_LOS_time_elapsed)

        df_B_LOS= pd.DataFrame({
            "seed_set_size": seed_set_size_constraint_list[ : idx_seed_set_size+1],
            "S_detected": L_B_LOS_S_detected,
            "S_timesteps": L_B_LOS_S_timesteps,
            "n_S": L_B_LOS_n_S,
            "n_S_correct": L_B_LOS_n_S_correct,
            "TP": L_B_LOS_TP,
            "TN": L_B_LOS_TN,
            "FP": L_B_LOS_FP,
            "FN": L_B_LOS_FN,
            "F1": L_B_LOS_F1,
            "MCC": L_B_LOS_MCC,
            "Time(s)": L_B_LOS_time_elapsed
            })

    print("\nB_LOS results")
    print(df_B_LOS.round(2))
    ...
    if args.dose_response == "exponential":
        ...
        outfile = "linear_B_LOS.csv"

    df_B_LOS.to_csv(path+outfile, index=False)
