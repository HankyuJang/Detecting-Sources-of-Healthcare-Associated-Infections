...
Experiment on larger number of simulation replicates

(3) In the proof of Theorem 4.8, the authors say that their approximation guarantee is achieved after just one iteration. So, if I understand this correctly, the rest of the iterations are superfluous from a worst-case approximation point of view. So, I think we should just run a small number of iterations if ISK and experiment a bit with how many iterations make empirical sense.
-> experiment on UIHC subgraph data and Karate to answer 3) ? Keep track of the quality of the solution at the end of each iteration and plot iteration # vs quality.

Usage

To run it on Karate graph,
$ python ISCK_approx_check.py -name Karate_temporal

To run it on UIHC sampled graph,
$ python ISCK_approx_check.py -name UIHC_HCP_patient_room -year 2011 -sampled True
...
# input: list of seed sets
# output: arrays w/ length of the iteration of ISCK
def prep_quality_arrays_for_plotting(list_of_seed_set):
    n_ISCK_iterations = len(list_of_seed_set)
    array_P_hit = np.zeros((n_ISCK_iterations))
    array_N_hit = np.zeros((n_ISCK_iterations))
    array_P_hit_frac = np.zeros((n_ISCK_iterations))
    array_N_hit_frac = np.zeros((n_ISCK_iterations))
    array_MCC = np.zeros((n_ISCK_iterations))
    # array_F1_frac = np.zeros((n_ISCK_iterations))

    for idx, S_detected in enumerate(list_of_seed_set):
        seeds_array = np.zeros((simul.n_timesteps, simul.number_of_nodes)).astype(bool)
        seeds_array[0, list(S_detected)] = True

        # Computing loss for ISCK
        _, _, probability_array = compute_loss_per_timestep(simul, seeds_array, obs_state_array)
        ISCK_n_S_correct = len(set(S_detected).intersection(set(S)))
        # compute hit ratios
        P_hit, N_hit, P_hit_frac, N_hit_frac, P_N_hit_diff, P_N_hit_ratio = get_pos_hit_neg_hit(nodes_in_P, nodes_in_N, probability_array)
        TP, TN, FP, FN = compute_TP_TN_FP_FN(P_hit, N_hit, len_P, len_N)
        MCC = get_MCC(TP, TN, FP, FN)

        array_P_hit[idx] = P_hit
        array_N_hit[idx] = N_hit
        array_P_hit_frac[idx] = P_hit_frac
        array_N_hit_frac[idx] = N_hit_frac
        array_MCC[idx] = MCC
    return array_P_hit, array_N_hit, array_P_hit_frac, array_N_hit_frac, array_MCC

def compute_TP_TN_FP_FN(P_hit, N_hit, len_P, len_N):
    TP = P_hit
    TN = len_N - N_hit
    FP = N_hit
    FN = len_P - P_hit
...
def prepare_result_dataframes():
    ...
    return df_GT, df_ISCK, \
            df_GT_loss_total, df_GT_loss_1, df_ISCK_loss_total, df_ISCK_loss_1

def save_result_dataframes(folder, name, dose_response, GT_quality):
    # Save datasets
    df_GT.to_csv("../tables/{}/{}/GT.csv".format(folder, name), index=False)
    df_ISCK.to_csv("../tables/{}/{}/ISCK.csv".format(folder, name), index=False)
...
def ISCK(simul, V, P, N, k1, k2, how):
    ...
    while True:
        ...
        print("G(): ", g(simul, S, P))
        print("f(): ", f(simul, S, N))
        print("S at the iteration: {}".format(S))
        S_over_iterations.append(S)
    return S, S_over_iterations
...
def run_ISCK_report_loss_per_timestep(how):
    ...
    S_detected_over_iterations_dict = dict()
    ...
    for k in k_range:
        ...
        S_detected, S_detected_over_iterations = ISCK(simul, V, P, N, k1, k2, how)
        S_detected_over_iterations_dict[k] = S_detected_over_iterations
    ...
    return ISCK_S_detected_dict, ISCK_n_S_correct_dict, ISCK_loss_1_dict, ISCK_loss_total_dict, \
             P_hit_dict, N_hit_dict, P_hit_frac_dict, N_hit_frac_dict, P_N_hit_diff_dict, P_N_hit_ratio_dict, \
             S_detected_over_iterations_dict
...
def prepare_GT_data(simul, k_range, people_nodes_idx, day0_people_idx_array, GT_quality):
    ...
    n_replicates = 100
    simul.set_n_replicates(n_replicates) # TODO: Added.
    for k in k_range:
    # for k, seeds_array in seeds_array_dict.items():
        while True: # Ensure that |P| is large enough. Maybe at least 3+k ?
            ...
            simul.set_seeds(seeds_array)
            simul.simulate()
            probability_array = simul.probability_array # TODO: Added.
            infection_array = simul.infection_array # TODO: Added.
            # for rep in range(n_replicates):
                # simul.simulate()
                # probability_array[rep, :, :] = simul.probability_array
                # infection_array[rep, :, :] = simul.infection_array
            ...
            elif GT_quality == "median":# TODO: Added.
                rep_idx_to_select = argmedian(MCC_array)

            P = set(infection_array[rep_idx_to_select,-1,:].nonzero()[0])
            print("GT. k={}, |P|={}".format(k, len(P)))
            N = V - P
            P_dict[k] = P
            N_dict[k] = N
            obs_state = infection_array[rep_idx_to_select,:,:]
    ...
    return S_original_dict, seeds_array_dict, obs_state_dict, I1_dict, P_dict, N_dict , MCC_array
...
if __name__ == "__main__":
    ...
    n_timesteps = 31
    n_replicates = 1 # we'll change this later in the script
    rho = pow(math.e, -1)
    d = pow(math.e, -4)
    q = 1.0
    pi = 1.0
    ...
    # k_range = [2, 4, 6]
    k_range = [1,2,3]
    S_original_dict, seeds_array_dict, obs_state_dict, I1_dict, P_dict, N_dict , MCC_array = prepare_GT_data(simul, k_range, people_nodes_idx, day0_people_idx_array, GT_quality)
    ...
    # Compute and save arrays for plotting
    # For each k in [1, 2, 3] prepare the following:
    for k in k_range:
        list_of_seed_set = S_detected_over_iterations_dict[k]
        P = P_dict[k]
        N = N_dict[k]
        len_P = len(P)
        len_N = len(N)
        ...
        array_P_hit, array_N_hit, array_P_hit_frac, array_N_hit_frac, array_MCC = prep_quality_arrays_for_plotting(list_of_seed_set)
        # Save the arrays
        np.savez("../npz/ISCK_approx_check/{}/arrays_k{}".format(name, k),
                array_P_hit=array_P_hit,
                array_N_hit=array_N_hit,
                array_P_hit_frac=array_P_hit_frac,
                array_N_hit_frac=array_N_hit_frac,
                array_MCC=array_MCC
                )

    ####################################################################
    # 5. Save results 
    folder = "ISCK_approx_check"
    df_GT, df_ISCK, \
            df_GT_loss_total, df_GT_loss_1, df_ISCK_loss_total, df_ISCK_loss_1 = prepare_result_dataframes()
    ...
    save_result_dataframes(folder, name, dose_response, GT_quality)
