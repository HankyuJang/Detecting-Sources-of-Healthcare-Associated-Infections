...
if __name__ == "__main__":
    ...
    if args.dose_response=="exponential":
        infile = "F1best_result_concat.csv"
    elif args.dose_response=="linear":
        infile = "F1best_linear_result_concat.csv"
    ...
    P2_LazyISCK_greedy_F1 = df_result[df_result["Algorithm"].isin(P2_LazyISCK_greedy_list)]["F1"].values[0]
    ...
    P2_LazyISCK_multiplicative_update_F1 = df_result[df_result["Algorithm"].isin(P2_LazyISCK_multiplicative_update_list)]["F1"].values[0]
    ...
    if P2_LazyISCK_greedy_F1 > P2_LazyISCK_multiplicative_update_F1:
    ...
    P3_GreedyRatio_F1 = df_result[df_result["Algorithm"].isin(P3_GreedyRatio_list)]["F1"].values[0]
    ...
    P3_GreedyRatio_ghit50_F1 = df_result[df_result["Algorithm"].isin(P3_GreedyRatio_ghit50_list)]["F1"].values[0]
    P3_GreedyRatio_ghit50 = df_result[df_result["Algorithm"].isin(P3_GreedyRatio_ghit50_list)]["Algorithm"].values[0]

    if P3_GreedyRatio_F1 > P3_GreedyRatio_ghit50_F1:
    ...
    F1_best_our_method = np.max(df_result_bar[df_result_bar["Algorithm"].isin(["KnapSackSD", "RatioSD"])]["F1"].values)
    F1_best_other_method = np.max(df_result_bar[~df_result_bar["Algorithm"].isin(["KnapSackSD", "RatioSD"])]["F1"].values)
    print("F1 our method: {:.2f}".format(F1_best_our_method))
    print("F1 other method: {:.2f}".format(F1_best_other_method))
    print("our_method/other_method {:.2f}\n".format(F1_best_our_method/F1_best_other_method))
    ...
    plot_bar_v2(GT_F1, Random_F1, df_result_bar["F1"].values, df_result_bar["Algorithm"].values, title, outpath, outfile)
