...
The chosen parameter set is saved as `tables/parameter_tuning/{}_k{}.csv".format(graph_name, k_total)`

Usage

$ python parameter_tuning_others.py -name G_UVA -seeds_per_t 1
$ python parameter_tuning_others.py -name G_UVA -seeds_per_t 2
$ python parameter_tuning_others.py -name G_UVA -seeds_per_t 3


$ python parameter_tuning_others.py -name G_Carilion -seeds_per_t 1
$ python parameter_tuning_others.py -name G_Carilion -seeds_per_t 2
...
if __name__ == "__main__":
    ...
    # parameter_list_contact_area = [500, 1000]
    # parameter_list_rho = [0.5, 1]
    # parameter_list_d = [0.1, 0.2]
    # parameter_list_q = [0.5, 1]
    # parameter_list_pi = [0.5, 1]

    # parameter_list_contact_area = [2000*pow(math.e, -i) for i in range(5)] # [2000.0, 735.7588823428847, 270.6705664732254, ...]
    parameter_list_contact_area = [2000*pow(math.e, -i) for i in range(1, 5)] # [735.7588823428847, 270.6705664732254, ...]
    ...
    for contact_area in parameter_list_contact_area:
        ...
        for rho in parameter_list_rho:
            ...
            for d in parameter_list_d:
                ...
                df_results["P_T/(P_T+N_T)"] = df_results["P_T(avg)"] / (df_results["P_T(avg)"] + df_results["N_T(avg)"])
                print(df_results[["trans-eff", "die-off", "shedding", "infectivity", "A(contact)", "P_T(avg)", "N_T(avg)", "P_T/(P_T+N_T)"]])

    # process final results
    df_results.to_csv("../tables/parameter_tuning/total_{}_k{}.csv".format(graph_name, k_total), index=False)

    # df_results_sub = df_results[(df_results["P_T/(P_T+N_T)"] >= 0.05) & (df_results["P_T/(P_T+N_T)"] < 0.1)]
    df_results_sub = df_results[(df_results["P_T/(P_T+N_T)"] >= 0.03) & (df_results["P_T/(P_T+N_T)"] < 0.1)]

    if df_results_sub.shape[0] == 0:
        print("No parameter setting yields 3-10% infection at P_T!")

    chosen_parameters = df_results_sub.sort_values(by="Recover_cnt(avg)", ascending=False).iloc[0]
    df_chosen_parameters = pd.DataFrame(chosen_parameters[:5]).T
    print(df_chosen_parameters)

    df_chosen_parameters.to_csv("../tables/parameter_tuning/{}_k{}.csv".format(graph_name, k_total), index=False)
