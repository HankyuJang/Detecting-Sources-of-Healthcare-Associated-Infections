...
results saved in `tables/lazy_greedy_exp/`
...
$ python lazy_greedy_experiment.py -seeds_per_t 1

To run it on UIHC sampled graph,
$ python lazy_greedy_experiment.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -sampled True -seeds_per_t 1

To run it on UIHC original graph,
$ python lazy_greedy_experiment.py -name UIHC_HCP_patient_room_withinHCPxPx -year 2011 -seeds_per_t 1
"""
...
def prepare_result_dataframes():
    ...
    return df_GT, df_BR, df_G1, df_G1_Lazy

def concat_result_dataframes():
    index_list = ["GT", "Random", "P1-Greedy", "P1-LazyGreedy"]
    df_result = pd.concat([df_GT, df_BR, df_G1, df_G1_Lazy])
...
def save_result_dataframes(name, k_total):
    # Save datasets
    df_GT.to_csv("../tables/lazy_greedy_exp/{}/k{}/GT.csv".format(name, k_total), index=False)
    df_BR.to_csv("../tables/lazy_greedy_exp/{}/k{}/BR.csv".format(name, k_total), index=False)
    df_G1.to_csv("../tables/lazy_greedy_exp/{}/k{}/G1.csv".format(name, k_total), index=False)
    df_G1_Lazy.to_csv("../tables/lazy_greedy_exp/{}/k{}/G1_Lazy.csv".format(name, k_total), index=False)

    df_result.to_csv("../tables/lazy_greedy_exp/{}/k{}/result_concat.csv".format(name, k_total), index=True)
    df_ground_truth_observations.to_csv("../tables/lazy_greedy_exp/{}/k{}/GT_observations.csv".format(name, k_total), index=False)
...
def print_result_dataframes():
    ...
    print(df_G1.round(2))
    print("\nProblem1: Lazy Greedy w/ cardinality constraint")
...
if __name__ == "__main__":
    ...
    print("Greedy1: optimize on loss_1")
    focus_obs1 = True
    G1_seeds_array, G1_n_S, G1_n_S_correct, G1_loss_1, G1_loss_total, \
        ...
        G1_TP, G1_TN, G1_FP, G1_FN, G1_F1, G1_MCC = \
            run_greedy_source_detection_report_loss_per_timestep(simul, focus_obs1, list_of_people_idx_arrays, number_of_seeds_over_time, \
                seeds_array, obs_state, MTP, list_of_sets_of_P, list_of_sets_of_N, n_t_for_eval, flag_lazy=False)
    stop = timeit.default_timer()
    ...
    df_GT, df_BR, df_G1, df_G1_Lazy = prepare_result_dataframes()
